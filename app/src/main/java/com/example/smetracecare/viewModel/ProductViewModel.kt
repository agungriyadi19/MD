...
class ProductViewModel : ViewModel() {
    ...
    val product: LiveData<List<ProductDetail>> get() = _product

    fun GetProduct(token:String, userId: String) {
        _loading.value = true
        val api = ApiConfig.getApiService().getProduct(token, userId)
        api.enqueue(object : retrofit2.Callback<GetProduct> {
            override fun onResponse(call: Call<GetProduct>, response: Response<GetProduct>) {
                _loading.value = false
                val responseBody = response.body()
                if (response.isSuccessful) {
                    isError = false
//                    _product.value = responseBody!!
                    response.body()?.let {
                        _product.value = it.result
                    }
                } else {
                    isError = true
